...
export default function ProductCard({ product, viewMode }: ProductCardProps) {
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 shadow-md hover:shadow-lg transition-all duration-300 mt-8 mb-4 overflow-hidden ${isGridView ? 'w-full' : 'flex'
      } hover:bg-slate-50`}>
      <div className={`relative pt-8 px-4 pb-4 ${isGridView ? 'h-72' : 'h-64 w-64'}`}>
        <div className="w-full h-full rounded-md overflow-hidden relative">
          <Image
            ...
            sizes={isGridView ? "(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" : "256px"}
          />
        </div>
